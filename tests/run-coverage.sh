...
if ! pip show pytest-cov >/dev/null 2>&1; then
    ...
    pip install -r "$SCRIPT_DIR/test-config/requirements-dev.txt"
fi

# Clean up any existing test artifacts
rm -f "$SERVICE_ROOT/.coverage"
rm -rf "$SERVICE_ROOT/.benchmarks"
...
PYTHONPATH="$WORKSPACE_ROOT:$SERVICE_ROOT" pytest \
    --rootdir=. \
    --confcutdir=. \
    -c "$SCRIPT_DIR/test-config/pytest.ini" \
    --cov=app \
